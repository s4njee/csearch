...
func parseBillXML(path string, congress int) (ParsedBill, error) {
	data, err := os.ReadFile(path)
	if err != nil {
		return ParsedBill{}, err
	}
...
func buildParsedBill(
	...
	bill := csearch.InsertBillParams{
		...
		PolicyArea:        nullStr(subjects.BillSubjects.PolicyArea.Name),
	...
	parsedCommittees := make([]csearch.InsertBillCommitteeParams, 0, len(committees.BillCommittees.Items))
	for _, committee := range committees.BillCommittees.Items {
	...
	parsedSubjects := make([]csearch.InsertBillSubjectParams, 0, len(subjects.BillSubjects.LegislativeSubjects.Items))
	for _, subject := range subjects.BillSubjects.LegislativeSubjects.Items {
		if subject.Name == "" {
			continue
		}
...
func processBills(ctx context.Context, db *sql.DB, queries *csearch.Queries, cfg appConfig, hashes *fileHashStore) error {
	for congress := 93; congress <= 118; congress++ {
		for _, table := range billTables {
			...
			parsedBills := collectChangedBills(jobs, hashes)
			for _, parsedBill := range parsedBills {
				insertParsedBill(ctx, db, queries, parsedBill)
			}
		}
	}
...
func billJobsForTable(cfg appConfig, congress int, table string) ([]billJob, error) {
	directory := filepath.Join(cfg.CongressDir, "data", fmt.Sprintf("%d", congress), "bills", table)
	...
	for _, entry := range entries {
		...
		jobs = append(jobs, billJob{
			Path: xmlPath,
			Parse: func(path string) (ParsedBill, error) {
				return parseBillXML(path, currentCongress)
			},
			Display: filepath.Base(base),
		})
	}
...
// collectChangedBills runs parser jobs concurrently and returns all bills
// whose source files have changed since the last successful ingest.
func collectChangedBills(jobs []billJob, hashes *fileHashStore) []ParsedBill {
	parsedBills := make([]ParsedBill, 0, len(jobs))
	var mu sync.Mutex
	...
	for _, job := range jobs {
		...
		go func() {
			...
			mu.Lock()
			parsedBills = append(parsedBills, parsedBill)
			mu.Unlock()
...
func insertParsedBill(ctx context.Context, db *sql.DB, queries *csearch.Queries, parsedBill ParsedBill) {
	tx, err := db.BeginTx(ctx, nil)
	if err != nil {
		log.Printf("BeginTx failed for %s-%s-%s: %v", parsedBill.Bill.Congress, parsedBill.Bill.Billtype, parsedBill.Bill.Billnumber, err)
		return
	}
	defer tx.Rollback()
	...
	if err := q.InsertBill(ctx, parsedBill.Bill); err != nil {
		log.Printf("InsertBill failed for %s-%s-%s: %v", parsedBill.Bill.Congress, parsedBill.Bill.Billtype, parsedBill.Bill.Billnumber, err)
		return
	}
	...
	if err := q.DeleteBillActions(ctx, key); err != nil {
		log.Printf("DeleteBillActions failed: %v", err)
		return
	}
	...
	for _, action := range parsedBill.Actions {
		...
		if err != nil {
			log.Printf("InsertBillAction failed: %v", err)
			return
		}
	...
	if latestActionID != 0 {
		if err := q.UpdateBillLatestAction(ctx, csearch.UpdateBillLatestActionParams{
			...
			log.Printf("UpdateBillLatestAction failed: %v", err)
			return
		}
	} else if err := q.ClearBillLatestAction(ctx, csearch.ClearBillLatestActionParams{
		Billtype:         key.Billtype,
		Billnumber:       key.Billnumber,
		Congress:         key.Congress,
		LatestActionDate: nullStr(parsedBill.LatestActionDate),
	}); err != nil {
		log.Printf("ClearBillLatestAction failed: %v", err)
		return
	}

	cosponsorKey := csearch.DeleteBillCosponsorsParams(key)
	if err := q.DeleteBillCosponsors(ctx, cosponsorKey); err != nil {
		log.Printf("DeleteBillCosponsors failed: %v", err)
		return
	}
	for _, cosponsor := range parsedBill.Cosponsors {
		if err := q.InsertBillCosponsor(ctx, cosponsor); err != nil {
			log.Printf("InsertBillCosponsor failed: %v", err)
			return
		}
	}
	...
	if err := q.DeleteBillSubjects(ctx, subjectKey); err != nil {
		log.Printf("DeleteBillSubjects failed: %v", err)
		return
	}
	for _, subject := range parsedBill.Subjects {
		if err := q.InsertBillSubject(ctx, subject); err != nil {
			log.Printf("InsertBillSubject failed: %v", err)
			return
		}
	}

	if err := tx.Commit(); err != nil {
		log.Printf("Commit failed for %s-%s-%s: %v", parsedBill.Bill.Congress, parsedBill.Bill.Billtype, parsedBill.Bill.Billnumber, err)
	}
}

func updateBills(cfg appConfig) error {
	// The double govinfo run mirrors the historical script behavior: one broad
	// sync plus an explicit current-congress refresh before materializing bills.
	commands := [][]string{
		{"govinfo", "--bulkdata=BILLSTATUS"},
		{"govinfo", "--bulkdata=BILLSTATUS", "--congress=118"},
		{"bills"},
	}

	for _, args := range commands {
		if err := runCongressTask(cfg, args...); err != nil {
			return err
		}
	}
